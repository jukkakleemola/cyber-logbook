...
CREATE TABLE xyz123_reservations (
    ...
    reserver_token UUID REFERENCES xyz123_users(user_token) ON DELETE CASCADE, -- Pseudonym reference
    resource_id INT REFERENCES xyz123_resources(resource_id),
...
CREATE TABLE xyz123_admin_logs (
    log_id SERIAL PRIMARY KEY,
    admin_id INT REFERENCES xyz123_users(user_id),
...
CREATE TRIGGER xyz123_check_age_trigger
BEFORE INSERT ON xyz123_reservations
FOR EACH ROW
EXECUTE FUNCTION xyz123_check_age();

-- View for anonymous access: Shows booked resources without reserver’s identity (pseudonymized view)
CREATE VIEW xyz123_booked_resources_view AS
SELECT
    r.resource_name,
    res.reservation_start,
    res.reservation_end
FROM xyz123_resources r
JOIN xyz123_reservations res ON r.resource_id = res.resource_id;

-- Deletion function for the right to erasure (compliant with GDPR)
CREATE OR REPLACE FUNCTION xyz123_erase_user(user_id_to_erase INT) RETURNS VOID AS $$
DECLARE
...
BEGIN
    ...
    DELETE FROM xyz123_users WHERE user_id = user_id_to_erase;
