...
async function isUniqueUsername(email) {
    const result = await client.queryArray(`SELECT username FROM xyz123_users WHERE username = $1`, [email]);
...
export async function registerUser(c) {
    ...
    try {
        ...
        await client.queryArray(`INSERT INTO xyz123_users (username, password_hash, role, birthdate, terms_accepted) VALUES ($1, $2, $3, $4, TRUE)`, [username, hashedPassword, role, birthdate]);
...
export async function getAccountInfo(username) {
    try {
        ...
        const result = await client.queryObject(
            `SELECT username, role, terms_accepted, created_at FROM xyz123_users WHERE username = $1`,
            [username]
        );
